...
public class FlujoEnVTests
{
    ...
    {
        // Arrange
        var domain = Types.InAssembly(Layers.DomainAssembly);

        // Act
        var result = domain.Should()
    ...
    {
        // Arrange
        var application = Types.InAssembly(Layers.ApplicationAssembly);

        // Act
        var result = application.Should()
    ...
    {
        // Arrange
        var infrastructure = Types.InAssembly(Layers.InfrastructureAssembly);

        // Act
        var result = infrastructure.Should().NotHaveDependencyOn(Layers.Api).GetResult();
    ...
    public void Controllers_ShouldOnlyTalkToTheMediator()
    {
        ...
        var result = controllers.Should()
            .NotHaveDependencyOnAny(Layers.Ports, Layers.Services, Layers.Infrastructure, "Microsoft.EntityFrameworkCore")
            .GetResult();
